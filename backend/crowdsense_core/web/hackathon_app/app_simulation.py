...
from simulation.crowdsense_simulation import get_dashboard_data, initialize_system
...
@app.route("/")
def index():
    """Render the main dashboard with simulation controls"""
    try:
        data = get_dashboard_data()
        return render_template("index_simulation.html", data=data)
        ...
        return render_template("index_simulation.html", data=error_data)
...
def trigger_simulation():
    """API endpoint to trigger disaster scenarios"""
    try:
        ...
        return jsonify({
            'success': True,
            'scenario': scenario,
            'message': f"{severity.title()} {disaster_type} triggered in {scenario['location']}"
        })
    except Exception as e:
        return jsonify({
            'success': False,
...
def active_scenarios():
    """API endpoint to get active scenarios"""
    try:
...
def start_sim():
    ...
    try:
        start_simulation_mode()
        return jsonify({'success': True, 'message': 'Simulation mode started'})
    except Exception as e:
...
def stop_sim():
    ...
    try:
        stop_simulation_mode()
        return jsonify({'success': True, 'message': 'Simulation mode stopped'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
